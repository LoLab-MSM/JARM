...
# new_nominal = np.load('jnk3_noASK1_calibrated_pars_pso26_1h.npy')

def run_example():
    pso = PSO(save_sampled=False, verbose=True, num_proc=4)
    pso.set_cost_function(likelihood)
    pso.set_start_position(xnominal + np.random.uniform(-1, 1, len(xnominal)))
    pso.set_bounds(lower=lower, upper=upper)
    pso.set_speed(-.25, .25)
    pso.run(25, 100)
    display(pso.best)
    np.save('jnk3_noASK1_calibrated_pars_pso_1h', pso.best)
